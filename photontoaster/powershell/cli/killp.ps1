...
if (-not $matches) {
  if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
    Write-PTMsg "killp: no processes match pattern: $Pattern" Err
  }
  exit 1
}
...
if ($List) {
  if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
    Write-PTMsg "killp: candidates (no stop yet)" Info
  }
...
if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
  Write-PTMsg "killp: sent stop to $($matches.Count) process(es)" Ok
}