...
function _bl_icon($item) {
  if ($item.PSIsContainer) { return "≡ƒôü" }
  if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) { return "≡ƒöù" }

  $ext = ($item.Extension ?? "").ToLowerInvariant()
  if ($ext -in ".exe",".bat",".cmd",".ps1",".sh") { return "ΓÜÖ∩╕Å" }
  if ($ext -in ".dll") { return "≡ƒº⌐" }
  if ($ext -in ".png",".jpg",".jpeg",".gif",".webp",".tga",".bmp") { return "≡ƒû╝∩╕Å" }
  if ($ext -in ".zip",".7z",".rar",".tar",".gz") { return "≡ƒôª" }
  if ($ext -in ".md",".txt",".log") { return "≡ƒô¥" }
  if ($ext -in ".json",".yml",".yaml",".toml",".xml") { return "≡ƒº╛" }
  if ($ext -in ".cs",".cpp",".c",".h",".hpp",".js",".ts",".py",".rs",".go") { return "≡ƒÆ╗" }
  return "≡ƒôä"
}
...
if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
  Write-PTMsg "Installed better ls/ll into your profile:" Ok
  Write-PTMsg $PROFILE Info
  Write-PTMsg "Reload:  . `$PROFILE" Accent
  Write-PTMsg "Then try:  ls   ll" Muted
} else {
