...
if (-not (Test-Path -LiteralPath $p)) {
  if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) { Write-PTMsg "reload: no profile at $p" Warn }
  else { Write-Warning "No profile at $p" }
  return
}

. $p
if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) { Write-PTMsg "reload: $p" Ok }