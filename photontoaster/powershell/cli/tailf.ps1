...
if (-not (Test-Path -LiteralPath $Path)) {
  if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
    Write-PTMsg "tailf: not found: $Path" Err
  }
  exit 1
}
...
if (Get-Command Write-PTMsg -ErrorAction SilentlyContinue) {
  ...
  if (Get-Command Format-PTPathLink -ErrorAction SilentlyContinue) {
    try { $disp = Format-PTPathLink -Path $Path -Display ((Resolve-Path -LiteralPath $Path).Path) } catch { }
  }
  Write-PTMsg ("tailf: following last $Count lines of " + $disp) Info
}
