...
- **PhotonToaster junction** (this folder on PATH): from `powershell\PhotonToaster` run `.\install.ps1`.
- **Native CLIs (winget)**: run `..\winget\Install-Extern.ps1` or `..\install.ps1` (full PhotonToaster setup runs both).
...
| `locate` | System-wide filename search ΓÇö instant via Everything (`es.exe`), falls back to `Get-ChildItem` |
