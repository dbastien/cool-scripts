<#
.SYNOPSIS
Benchmark PowerShell startup time (with and without PhotonToaster profile).
.DESCRIPTION
Uses hyperfine when available; falls back to a pure PowerShell Stopwatch loop.
Shows the delta between a bare pwsh and pwsh+profile to quantify overhead.
.EXAMPLE
bench
bench -Runs 20
...
if (Get-Command hyperfine -ErrorAction SilentlyContinue) {
  ...
  $profile_ = _bench_loop 'pwsh+profile' @('-i', '-c', 'exit') $Runs $Warmup
  Write-Host ''
  ...
  Write-Host "${e}[38;2;${color}mProfile overhead: ${delta}ms${e}[0m"
