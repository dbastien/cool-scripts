<#
.SYNOPSIS
  Instellator entry: winget CLIs (Install-Extern), Toasty junction, optional GuiApps / FirefoxExt / ChromiumExt.

.DESCRIPTION
  Run from this folder or via `toasty\install.ps1` (which calls winget\Install-Extern.ps1 then does the Toasty junction).
#>
param(
  [switch]$Force,
...
$externScript = Join-Path $ptRoot 'winget\Install-Extern.ps1'
...
if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
  Write-ToastyMsg 'instellator: step 1 - native CLIs (full PT-aligned winget set; use -MinimalExtern for a smaller subset)' Accent
} else {
  Write-Host '=== instellator: native CLIs ==='
}
& $externScript @externArgs

if ($WhatIf) {
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg 'instellator: -WhatIf skips install.ps1 and GUI helpers (re-run without -WhatIf).' Warn
  } else {
    Write-Warning 'instellator: -WhatIf skips install.ps1.'
  }
} else {
  Sync-PathFromRegistry

  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg 'instellator: step 2 - junction Toasty into ~/.config/toasty' Accent
  } else {
    Write-Host '=== instellator: Toasty junction ==='
  }
  ...
  if ($GuiAppsAuto -and (Test-Path -LiteralPath $guiScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - GuiApps.ps1 -AutoDefault' Accent
    }
    & $guiScript -AutoDefault
  } elseif ($GuiApps -and (Test-Path -LiteralPath $guiScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - GuiApps.ps1' Accent
    }
    & $guiScript
  }
  ...
  if ($FirefoxExtensionsAuto -and (Test-Path -LiteralPath $firefoxScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - FirefoxExt.ps1 -AutoDefault' Accent
    }
    & $firefoxScript -AutoDefault
  } elseif ($FirefoxExtensions -and (Test-Path -LiteralPath $firefoxScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - FirefoxExt.ps1' Accent
    }
    & $firefoxScript
  }
  ...
  if ($ChromiumExtensionsAuto -and (Test-Path -LiteralPath $chromiumScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - ChromiumExt.ps1 -AutoDefault' Accent
    }
    & $chromiumScript -AutoDefault
  } elseif ($ChromiumExtensions -and (Test-Path -LiteralPath $chromiumScript)) {
    if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
      Write-ToastyMsg 'instellator: optional - ChromiumExt.ps1' Accent
    }
    & $chromiumScript
  }
}
...
Write-Host ''
if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
  Write-ToastyMsg 'instellator: done. Open a new terminal (Windows Terminal recommended) so PATH changes apply.' Ok
  Write-ToastyMsg 'Profile was patched automatically. To re-run:  ..\toasty\shell\install-profile.ps1' Muted
  Write-ToastyMsg 'Optional Fira Code Nerd Font: re-run with -NerdFontFiraCode or tick it in Instellator\GuiApps.ps1.' Muted
  ...
  Write-ToastyMsg 'Instellator: GuiApps.ps1  |  Firefox .xpi: FirefoxExt.ps1  |  Chrome/Edge: ChromiumExt.ps1' Muted
} else {
