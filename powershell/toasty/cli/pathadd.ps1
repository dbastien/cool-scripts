...
try {
  ...
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg "pathadd: $Dir : $($_)" Err
  }
  exit 1
}
...
if (-not ($env:Path -split ';' | Where-Object { $_ -eq $full })) {
  ...
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg "Prepended PATH: $full" Ok
  }
} elseif (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
  Write-ToastyMsg "PATH already contains: $full" Info
}