...
if (-not (Test-Path -LiteralPath $Root)) {
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg "du: not found: $Root" Err
  }
  exit 1
}
...
if (Test-Path -LiteralPath $Root -PathType Leaf) {
  ...
  if (Get-Command Format-ToastyPathLink -ErrorAction SilentlyContinue) {
    $pathDisp = Format-ToastyPathLink -Path $Root -Display $pathDisp
  }
  $c = $null
  if (Get-Command Get-ToastyDuColor -ErrorAction SilentlyContinue) { $c = Get-ToastyDuColor $mib }
...
Get-ChildItem -LiteralPath $Root -Directory -Force -ErrorAction SilentlyContinue |
  ...
  $pathDisp = $_.Path
  if (Get-Command Format-ToastyPathLink -ErrorAction SilentlyContinue) {
    $pathDisp = Format-ToastyPathLink -Path $_.Path -Display $_.Path
  }
  $c = $null
  if (Get-Command Get-ToastyDuColor -ErrorAction SilentlyContinue) { $c = Get-ToastyDuColor $_.MiB }
