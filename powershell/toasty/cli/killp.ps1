...
if (-not $matches) {
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg "killp: no processes match pattern: $Pattern" Err
  }
  exit 1
}
...
if ($List) {
  if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
    Write-ToastyMsg "killp: candidates (no stop yet)" Info
  }
...
if (Get-Command Write-ToastyMsg -ErrorAction SilentlyContinue) {
  Write-ToastyMsg "killp: sent stop to $($matches.Count) process(es)" Ok
}