# .SYNOPSIS
# Registers (or removes) the per-user toasty-cd: URL protocol so Ctrl+click on rewritten eza links copies a cd line.
...
$root = 'HKCU:\Software\Classes\toasty-cd'
$handler = Join-Path $PSScriptRoot 'Invoke-ToastyCdUrl.ps1'
...
New-ItemProperty -Path $root -Name '(Default)' -Value 'URL:Toasty cd (clipboard)' -PropertyType String -Force | Out-Null
...
Write-Host "Registered toasty-cd: -> $handler"
