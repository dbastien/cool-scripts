# Toasty "did you mean?" handler — suggests similar commands on typos.
# Requires PowerShell 7.4+ for CommandNotFoundAction; silently skips on older versions.

$global:_toastyCmdCache = $null

function _toasty_levenshtein([string]$s, [string]$t) {
...
function _toasty_get_cmd_names {
  if ($null -eq $global:_toastyCmdCache) {
    $global:_toastyCmdCache = @(Get-Command -Type All -ErrorAction SilentlyContinue |
      ForEach-Object { $_.Name } | Sort-Object -Unique)
  }
  return $global:_toastyCmdCache
}

function _toasty_invalidate_cmd_cache {
  $global:_toastyCmdCache = $null
}

function _toasty_find_suggestions([string]$typo, [int]$maxResults = 6) {
  $names = _toasty_get_cmd_names
  $tLen = $typo.Length
  ...
  foreach ($name in $names) {
    ...
    $dist = _toasty_levenshtein $tLower $nLower
...
if ($PSVersionTable.PSVersion -ge [version]'7.4') {
  $ExecutionContext.InvokeCommand.CommandNotFoundAction = {
    ...
    $suggestions = _toasty_find_suggestions $name
