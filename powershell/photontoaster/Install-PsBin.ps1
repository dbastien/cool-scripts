...
$ptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
...
# Flatten cli/*.ps1 plus shell helpers (quote + aliases) into psbin; skip paths under TargetDir.
$dopeFiles = Get-ChildItem -LiteralPath $cliDir -Filter "*.ps1" -File -Recurse | Where-Object {
  (-not $targetResolved -or (-not $_.FullName.StartsWith($targetResolved, [System.StringComparison]::OrdinalIgnoreCase)))
}
$shell = Join-Path $ptRoot 'shell'
$extraPaths = @(
  (Join-Path $shell 'QuoteOfDay.ps1')
  (Join-Path $shell 'ShortPs1Prompt.ps1')
  (Join-Path $ptRoot 'lib\ShellAliases.ps1')
)
...
if (-not $allPs1) {
  if (Get-Command Write-ShortPs1Msg -ErrorAction SilentlyContinue) {
    Write-ShortPs1Msg "No .ps1 files found under: $cliDir" Warn
  } else {
    Write-Host "No .ps1 files found under: $cliDir"
  }
  exit 0
}
...
# Seed quotes for Show-ShortPs1QuoteOfDay if missing (same path as before for compatibility)
...
if (-not (Test-Path -LiteralPath $quotesDest)) {
  $bundledQuotes = Join-Path $shell 'quotes.default.txt'
...
$promptBundled = Join-Path $shell 'prompt.config.default.toml'
