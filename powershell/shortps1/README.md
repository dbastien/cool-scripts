...
| Area | Folder | Role |
|------|--------|------|
| **Toasty** | `..\toasty\` | `cli\` (tools copied to `psbin`), `lib\` (`ShortCommon.ps1`, `ShellAliases.ps1`), `shell\` (quote, prompt, profile hooks, defaults), `Install-PsBin.ps1`, optional `Init.ps1` / `config.psd1` — see `..\toasty\README.md` |
...
Runtime styling and OSC8 helpers for the Toasty tools live in `..\toasty\lib\ShortCommon.ps1`. Thin wrappers at the `shortps1` root forward to the folders above (`install.ps1` → toasty `Install-PsBin.ps1`, `Install-Extern.ps1`, `Install-Full.ps1`, `Install-DevDependencies.ps1`) so `cd shortps1` one-liners stay short.

Winget package IDs for native CLIs are in `cli\WingetManifest.ps1`.

**Toasty** ships quotes and prompt defaults under `toasty\shell\` and installs into your profile data / `psbin` via `Install-PsBin.ps1`.

Disable the daily quote in PowerShell: `$env:SHORTPS1_NO_QUOTE = '1'`. Custom file: `$env:SHORTPS1_QUOTES_FILE`.

## Install (recommended)

One shot: **Windows Terminal** (winget) if `wt` is missing, then **native CLIs** (full manifest by default), then **Toasty** `cli\` scripts into `~/psbin`. Skips `jq.ps1` / `wget.ps1` when real `jq.exe` / `wget.exe` are on PATH after winget (avoids shadowing).
...
- `-WhatIf` — print winget actions only; **does not** run `..\toasty\Install-PsBin.ps1` or Instellator GUI helpers (re-run without `-WhatIf` to copy scripts).
...
- `-GuiApps` — after Toasty copy, run `Instellator\Install-GuiApps.ps1` (desktop apps picker).
...
Installs into `~/psbin` by default and adds it to your **User PATH** (won't overwrite existing files unless `-Force`). Installer scripts under `Instellator\`, `cli\`, `SharedLibs\`, `toasty\shell\Install-ProfileHooks.ps1`, etc., are not copied to `psbin` (only Toasty `cli\` tools plus `QuoteOfDay.ps1`, `ShortPs1Prompt.ps1`, seeded `prompt.config.toml` when missing, and `ShellAliases.ps1`).
...
```powershell
..\toasty\shell\Install-ProfileHooks.ps1
```

### ShortPs1 / Toasty prompt (optional)
...
..\toasty\shell\Install-ProfileHooks.ps1 -DopeShellPrompt
# prompt only (no quote block):
..\toasty\shell\Install-ProfileHooks.ps1 -DopeShellPrompt -SkipQuote
```
