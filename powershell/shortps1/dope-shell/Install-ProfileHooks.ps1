# Appends marked blocks to your PowerShell profile (quote of the day, optional dope-shell prompt).
# Run from repo:  pwsh -File .\dope-shell\Install-ProfileHooks.ps1
# Remove quote:    .\dope-shell\Install-ProfileHooks.ps1 -Remove
# Remove prompt:   .\dope-shell\Install-ProfileHooks.ps1 -RemovePrompt
# Add prompt:      .\dope-shell\Install-ProfileHooks.ps1 -DopeShellPrompt
# Prompt only:     .\dope-shell\Install-ProfileHooks.ps1 -DopeShellPrompt -SkipQuote
...
if (-not $SkipQuote) {
  if ($current -notmatch [regex]::Escape($startMarker)) {
    ...
    if ($WhatIf) {
      ...
      Write-Host "Ensure quotes exist at ~/.local/share/shortps1/quotes.txt (dope-shell install.ps1 seeds when missing)."
    }
  } else {
...
Write-Host "Appended dope-shell prompt (ShortPs1Prompt.ps1) to: $ProfilePath"
